...
System::System(string sConfig_file_)
    : bStart_backend(true)
{
    string sConfig_file = sConfig_file_ + "myslam_config.yaml";
...
void System::PubFeatureData(double dStampSec, const vector<int> &feature_id, const vector<Vector2d> &feature, const vector<Vector2d> &observation, std::vector<Vector2d> &featureVelocity)
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        // Skip the first image; since no optical speed on the first image
        if (!init_pub)
        {
        ...
        else
        {
            ...
            m_buf.unlock();
            con.notify_one(); // Randomly wake up a waiting thread
        }
    }
}
...
void System::PubImageData(double dStampSec, Mat &img)
{
    ...
    {
        cerr << "3 PubImageData image discontinuity! Reset the feature tracker!" << endl;
...
vector<pair<vector<ImuConstPtr>, ImgConstPtr>> System::getMeasurements()
{
    ...
    while (true)
    {
        // Stop if imu_buf or feature_buf is empty
        ...
        // Alignment condition: the last IMU data timestamp must be greater than the first image feature timestamp
        ...
        // Alignment condition: the first IMU data timestamp must be smaller than the first image feature timestamp
        if (!(imu_buf.front()->header < feature_buf.front()->header + estimator.td))
        {
            cerr << "Throw image, only should happen at the beginning" << endl;
            feature_buf.pop();
            continue;
        }
        ...
        {
            // emplace_back is better than push_back for avoiding unnecessary copies and moves
            IMUs.emplace_back(imu_buf.front());
            imu_buf.pop();
        }

        // Here, the next imu_msg is added but not popped,
        // so the current and next image frames will share this imu_msg
        // cout << "1 getMeasurements IMUs size: " << IMUs.size() << endl;
        IMUs.emplace_back(imu_buf.front());
        if (IMUs.empty())
        {
            cerr << "No imu between two image frames" << endl;
        }
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                {
                    // Handle the case when the timestamps do not match directly (interpolation)
                    ...
                    // Process the interpolated IMU data
                }
            }
            ...
            {
                // The 0.5 offset adjusts the feature ID for the camera
            ...
            // If the solver flag is set to NON_LINEAR, update the trajectory
...
void System::Draw()
{  
    // Create a Pangolin window to visualize the trajectory
    ...
    while (pangolin::ShouldQuit() == false)
    {
        ...
        usleep(5000); // Sleep for 5 ms to allow smooth rendering
    }
}
